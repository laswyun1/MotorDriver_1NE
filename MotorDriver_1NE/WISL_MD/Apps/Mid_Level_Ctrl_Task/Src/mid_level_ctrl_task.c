...
static void StateEnable_Ent( )
{
	...
	posCtrl.ref1 = posCtrl.ref;
	posCtrl.ref2 = posCtrl.ref;
...
static int Run_Disturbance_Obs()
{
	...
	if(posDOB.disturbance > posDOB.saturation)			{t_control_input = +posDOB.saturation;}
	else if(posDOB.disturbance < -posDOB.saturation)	{t_control_input = -posDOB.saturation;}
	else 												{t_control_input = +posDOB.disturbance;}

	if (posDOB.overall_gain_trigger == 1)
	{
		posDOB.overall_gain_curr = posDOB.overall_gain_curr + posDOB.overall_gain_gap;
		if (posDOB.overall_gain_time_stamp == MID_LEVEL_CONTROL_FREQUENCY * posDOB.overall_gain_transition_time)
		{
			posDOB.overall_gain_gap        = 0;
			posDOB.overall_gain_time_stamp = 0;
			posDOB.overall_gain_trigger    = 0;
			posDOB.overall_gain_curr       = posDOB.overall_gain_des;
		}
		posDOB.overall_gain_time_stamp++;

		if (posDOB.overall_gain_curr > 1) posDOB.overall_gain_curr = 1;
		if (posDOB.overall_gain_curr < 0) posDOB.overall_gain_curr = 0;
	}
	posDOB.control_input = t_control_input * posDOB.overall_gain_curr;
...
{
	/* For PD controller */
	posCtrl.ref = Generate_Sine(pos_periodic_sig.amp, mid_level_state.initial_pos + pos_periodic_sig.offset, pos_periodic_sig.freq, \
	...
	posFF.in[0] = Generate_Sine(pos_periodic_sig.amp, mid_level_state.initial_pos + pos_periodic_sig.offset, pos_periodic_sig.freq, \
			                    mid_level_loop_cnt, MID_LEVEL_CONTROL_PERIOD, 1)	\
			                     - mid_level_state.initial_pos;

	/*
	posFF.in[0] = Generate_Sine(pos_periodic_sig.amp, pos_periodic_sig.offset, pos_periodic_sig.freq, \
			                    mid_level_loop_cnt, MID_LEVEL_CONTROL_PERIOD, 1)	\
			                     - mid_level_state.initial_pos;
			                     */
