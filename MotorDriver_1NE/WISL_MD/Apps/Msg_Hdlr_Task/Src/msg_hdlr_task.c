...
//    uint16_t t_identifier = SDO|(MD_node_id<<4)|t_dest_node;
    uint16_t t_identifier = (0 << 10)|((MD_node_id & 0x1F) << 5)|(t_dest_node & 0x1F);
...
//    else 			{	t_identifier = PDO|(MD_node_id<<4)|t_dest_node;	}
    else            {   t_identifier = (1 << 10)|((MD_node_id & 0x1F) << 5)|(t_dest_node & 0x1F); }
...
//    ori_node = (t_wasp_id & 0x0F0)>>4;
    fnc_code = (t_wasp_id >> 10) & 0x01;
    ori_node = (t_wasp_id & 0x3E0) >> 5;
    des_node = t_wasp_id & 0x01F;
//    ori_node = (t_wasp_id >> 5) & 0x1F;

    if (des_node == MD_node_id) {
		switch(fnc_code) {
