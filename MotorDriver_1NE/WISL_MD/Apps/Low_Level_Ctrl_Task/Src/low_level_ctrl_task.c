...
static void Set_Current_LPF_ONOFF(MsgSDOargs* t_req, MsgSDOargs* t_res);
...
//	Init_Kalman_Filter(&kf_current_object, kf_current_object.kf_A, kf_current_object.kf_B, kf_current_object.kf_C, 1, 10, 1);	// A, B, C, Q, R, P (Q = 1, R = 10)
	...
	Create_SDO(TASK_ID_LOWLEVEL, SDO_ID_LOWLEVEL_CURRENT_LPF_ONOFF,             e_UInt8,    Set_Current_LPF_ONOFF);
...
static void Run_FOC_Ctrl(float t_current_in)
{
	...
	if(motor_setting.low_level_current_LPF_on == 1) {
		clarke_in.Iu = clarke_in.Iu*0.5 + t_iu*0.5; // 300Hz LPF
		clarke_in.Iv = clarke_in.Iv*0.5 + t_iv*0.5;
		clarke_in.Iw = clarke_in.Iw*0.5 + t_iw*0.5;
	}
	else
	{
		clarke_in.Iu = t_iu;
...
/*	clarke_in.Iu = t_iu;
	...
	//static int32_t park_out_Iq_filt;
	//static int32_t park_out_Id_filt;

/*	if(motor_setting.low_level_current_LPF_on == 1) {
		park_out_Iq_filt = 0.95*park_out_Iq_filt + 0.05*park_out.Iq;
		park_out_Id_filt = 0.95*park_out_Id_filt + 0.05*park_out.Id;
	}
	else
	{
		park_out_Iq_filt = park_out.Iq;
		park_out_Id_filt = park_out.Id;
	}*/
...
//	Run_PID_Control(&pid_d_axis_ctrl,			   0, park_out_Id_filt, LOW_LEVEL_CONTROL_PERIOD);
//	Run_PID_Control(&pid_q_axis_ctrl,	t_currentRef, park_out_Iq_filt, LOW_LEVEL_CONTROL_PERIOD); // ParkOut.Iq : Total Current
...
static void Set_Current_LPF_ONOFF(MsgSDOargs* t_req, MsgSDOargs* t_res)
{
	memcpy(&motor_setting.low_level_current_LPF_on,  t_req->data, 			 1);
...
static int Ent_Electrical_System_ID()
{
    ...
    motor_setting.low_level_current_LPF_on = 0;
